...
class Unit(Entity):
    def __init__(self, x, y, images_stand, images_walk, images_attack=None):
        ...
        self.target_y = y
        self.speed    = 4
        self.flip_x   = False   # True → смотрит влево
    ...
    def update(self, now, loot_list, obstacles):
        ...
        if self.state == "WALK":
            ...
            if not self._target_alive():
                # цель исчезла — сразу перейти в IDLE, не атаковать воздух
                self.state        = "IDLE"
                self.status_anims = self.images_stand
                self.target_target = None
                return
...
class Warrior(Unit):
    def __init__(self, x, y):
        ...
        self.explore_radius = 110   # уменьшен радиус круга
    ...
    def update(self, now, loot_list, obstacles, trees=None, mines=None):
        if (self.explore_point is not None
            ...
            if target:
                ...
                self.explore_point = None   # всё добыто — стоим
        super().update(now, loot_list, obstacles)


# ──────────────────────────────────────────────────────────────────────────────
# Arrow (снаряд лучника)
# ──────────────────────────────────────────────────────────────────────────────
...
class Arrow:
    ...
    def __init__(self, x, y, target):
        ...
        self.angle  = -math.degrees(math.atan2(-dy, dx)) - 45  # под 45° как у спрайта
        self.hit    = False
        self.hit_time = 0
        self.dead   = False
        # Спрайты: arrow_imgs[0] — летящая, arrow_imgs[1] — воткнувшаяся
    ...
    def update(self, now, loot_list):
        ...
        if math.hypot(self.x - tx, self.y - ty) < 20:
            ...
            # (destroyed не используем для стрел — лучник сам обновит цель)
...
class Archer(Unit):
    ...
    ATTACK_CD     = 1800  # мс между выстрелами
    ...
    def update(self, now, loot_list, obstacles, trees=None, mines=None):
        ...
        if self.state == "WALK":
            ...
            self.flip_x = dx < 0
            if dist <= self.RANGE:
                # В зоне обстрела — встать и стрелять
                self.state        = "ATTACK"
                self.status_anims = self.images_attack
                self.frame        = 0
            else:
...
class Building(Entity):
    """Базовый класс для всех зданий с поддержкой стройки."""
    BUILD_TIME = 10000   # мс — переопределяется в подклассах
    ...
    def __init__(self, x, y, img_construction, img_done, img_destroyed):
        ...
        self.builder          = None   # юнит-строитель (Warrior)
    ...
    def draw(self, surface, cam_x, cam_y):
        surface.blit(self.image, (self.x - cam_x, self.y - cam_y))
        if not self.built and not self.destroyed:
            # Полоска прогресса строительства
